(function () {
  ...
  function connect(){ socket=new WebSocket((location.protocol==='https:'?'wss:':'ws:')+'//'+location.host+'/ws'); socket.onopen=function(){status('请输入相同房间码');if(roomCode.length>=4)send({type:'join',roomId:roomCode});}; socket.onclose=function(){status('已断开，重连中…');setTimeout(connect,1200);}; socket.onmessage=function(e){var m=JSON.parse(e.data);if(m.type==='joined'){role=m.role;status('你控制：'+(role==='fire'?'火人':'冰人'));}else if(m.type==='room'){status(m.isReady?'队友已就绪，正在进入第一关':'等待另一台设备（1/2）');if(m.isReady)window.dispatchEvent(new Event('first-level:start'));}else if(m.type==='input'&&role==='fire')remoteInput=m.input;else if(m.type==='state'&&role==='water')apply(m.state);else if(m.type==='error')status(m.message);}; }
