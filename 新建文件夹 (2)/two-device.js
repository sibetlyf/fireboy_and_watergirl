(function () {
  ...
  function panel(){
    ...
    else{n.innerHTML='<strong>寄语模式 · 双设备合作</strong><p id="device-status">连接中…</p><section class="host-card"><b>创建专属贺卡</b><input id="card-to" maxlength="24" placeholder="贺卡给谁，例如：洛小白"><textarea id="card-message" maxlength="160" placeholder="写下想说的话"></textarea><input id="card-from" maxlength="24" placeholder="落款，例如：—— 与你同行"><select id="left-constellation" aria-label="左侧星座"><option value="taurus">左侧：金牛座 · Taurus</option><option value="aries">左侧：白羊座 · Aries</option><option value="gemini">左侧：双子座 · Gemini</option><option value="cancer">左侧：巨蟹座 · Cancer</option><option value="leo">左侧：狮子座 · Leo</option><option value="virgo">左侧：处女座 · Virgo</option><option value="libra">左侧：天秤座 · Libra</option><option value="scorpio">左侧：天蝎座 · Scorpio</option><option value="sagittarius">左侧：射手座 · Sagittarius</option><option value="capricorn">左侧：摩羯座 · Capricorn</option><option value="aquarius">左侧：水瓶座 · Aquarius</option><option value="pisces">左侧：双鱼座 · Pisces</option></select><select id="right-constellation" aria-label="右侧星座"><option value="cancer">右侧：巨蟹座 · Cancer</option><option value="aries">右侧：白羊座 · Aries</option><option value="taurus">右侧：金牛座 · Taurus</option><option value="gemini">右侧：双子座 · Gemini</option><option value="leo">右侧：狮子座 · Leo</option><option value="virgo">右侧：处女座 · Virgo</option><option value="libra">右侧：天秤座 · Libra</option><option value="scorpio">右侧：天蝎座 · Scorpio</option><option value="sagittarius">右侧：射手座 · Sagittarius</option><option value="capricorn">右侧：摩羯座 · Capricorn</option><option value="aquarius">右侧：水瓶座 · Aquarius</option><option value="pisces">右侧：双鱼座 · Pisces</option></select><button id="create-room">生成房间并邀请</button></section><div class="join-card"><input id="device-room" maxlength="8" placeholder="输入邀请码加入"><button id="join-room">加入第一关</button></div><div id="invite-box" hidden><small>分享邀请链接</small><input id="invite-link" readonly><button id="copy-invite">复制邀请</button></div><small>房主控制火人，受邀玩家控制冰人</small>';}
    document.body.appendChild(n);var roomInput=n.querySelector('#device-room');if(roomInput)roomInput.value=roomCode;
    var create=n.querySelector('#create-room');if(create)create.onclick=function(){var draft={to:n.querySelector('#card-to')?.value,message:n.querySelector('#card-message')?.value,from:n.querySelector('#card-from')?.value,leftConstellation:n.querySelector('#left-constellation')?.value,rightConstellation:n.querySelector('#right-constellation')?.value};send({type:'create',mode:mode,card:draft});status('正在生成联机房间…');};
